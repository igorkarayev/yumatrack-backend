...
export const verifyTokenAndGetUser = async (token: string): Promise<User> => {
  const jwtPayload: JwtPayload = <JwtPayload>jwt.verify(token, config.JWT.SECRET);
  ...
  const user: User = await userRepository.findUserWithDetailsByUserIdOrFail(jwtPayload.id);

  const isTokenActive = user.tokens.some((t: Token) => t.active && token.includes(t.signedString));
